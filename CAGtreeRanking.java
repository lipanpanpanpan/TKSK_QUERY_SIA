...
//			}
			...
			while( (doc=fin.nextSpatialDocument()) != null&&queryNum<70){
//					if(queryNum<70)
//					{
//						queryNum++;
//						continue;
...
//					System.out.println(queryNum+"\t"+doc);
					...
					long extraTime=query(doc.getLoc(), new Vector<String>(doc.getTerms().keySet()), K,doc.vertexID);
					long end = System.currentTimeMillis();
//					System.out.println("RRR:"+(end-start-extraTime));
...
//			System.out.println(queryNum);
			...
			System.err.println("total:\t"+itemAccessed);
			System.err.println("AVG:\t"+(itemAccessed/(queryNum*1.0)));
...
//		fw.close();
	...
	private long query(Point qLoc, Vector<String> queryKwds, int K,int queryVertexID) throws IOException {
...
//				System.out.println("List:"+i+"____"+list.size());
...
//		// start CA algorithm
		long extraTime=0;
		CAGTreeTOPK alg = new CAGTreeTOPK(textLists, K,queryVertexID);
		long sss=System.currentTimeMillis();
		
		extraTime=alg.run();
		
		long eee=System.currentTimeMillis();
//		System.out.println("Run:"+(eee-sss-extraTime));
//		alg.printResult();
//		//System.out.println("item accessed "+ alg.getAccessedItem());
		itemAccessed += alg.getAccessedItem();
		System.out.println("SSS"+ alg.getAccessedItem());
		System.out.println("TT:\t"+itemAccessed);
		return extraTime;
	}
...
//		String queryFile=Configure.DATA_ROOT_FOLDER_PATH+Configure.QUERY_FILE_FOLDER+Configure.dataSize+"/"+Configure.ROAD_NETWORK+"_DataSize_"+Configure.dataSize+"_kwdNum_"+Configure.queryNum+".txt";

//		String queryFile=Configure.DATA_ROOT_FOLDER_PATH+Configure.QUERY_FILE_FOLDER+Configure.dataSize+"/"+Configure.ROAD_NETWORK+"_"+Configure.dataSize+".txt";
		String queryFile=Configure.DATA_ROOT_FOLDER_PATH+Configure.QUERY_FILE_FOLDER+Configure.dataSize+"/"+Configure.ROAD_NETWORK+"_DataSize_"+Configure.dataSize+"_kwdNum_"+Configure.queryNum+".txt";
		int K =Configure.TopK;
//		Scorer.alpha = (float) Configure.A_OF＿SPATIAL;
		System.out.println("DB\t"+dbName);
		
		CAGtreeRanking TATest = new CAGtreeRanking(dbName);
